...
def TV_Show(apiId):
    try :
        resultTitle, length, webSite, vote, seasons = getBasicInfo(apiId)
    ...
    try:
        credit = getCast(apiId)
            ...
            credit=""
    try:
        showlang=getSpokenLang(apiId)
    except sql_executor.NoResultsException:
        showlang=""
    imagers1, links1 = getSimilarShow(apiId)
    imagerc1, linkc1 = getComputeShow(apiId)
    if imagerc1 == "":
